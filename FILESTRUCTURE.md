...
* C++ source files:
  * `include/pybind11-numpy-example/pybind11-numpy-example.hpp` is the main
    C++ header that declares the interface of your library.
  * `src/pybind11-numpy-example.cpp` is the main file that implements this library.
  * `app/pybind11-numpy-example_app.cpp` is an executable that uses the library.
    This can e.g. be used to provide a command line interface for your project.
  * `tests/pybind11-numpy-example_t.cpp` contains the unit tests for the library.
  ...
  * `python/pybind11-numpy-example_python.cpp` is the source file
